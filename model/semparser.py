...
class SemParser(nn.Module):
    ...
    def compute_loss(self, word_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover, combinations, combination_labels, token_label):
        lstm_feat_out = self.lstm_rep(word_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover) # (batch_size, seq_len, lstm_hidden_size*2)
        # print(lstm_feat_out)
        mention_feat = self.mention_rep(lstm_feat_out) # (batch_size, seq_len, mention_size)

        #mention_score = self.mention_score(lstm_feat_out)
        #prob_mention = torch.argsort(mention_score, dim=1, descending=True)[:5]

        batch_size = word_inputs.size(0)
        mention_feat_size = mention_feat.size(2)
        # combs_for_each_token = []
        # loss = 0
        mention_pair_loss_list = Variable(torch.zeros((len(combinations[0]))))
        for comb_idx, (cur_comb, cur_comb_label) in enumerate(zip(combinations[0], combination_labels[0])):
            # print(cur_comb)
            feat_combines = mention_feat[:, torch.tensor(cur_comb)].squeeze(0)
            feat_combines = feat_combines.view(batch_size, -1, mention_feat_size*2)

            # print(feat_combines.size())
            # joint comb
            feat_join = torch.empty([batch_size, len(cur_comb), self.word_emb_dim])
            for idx, pair in enumerate(cur_comb):
                if pair[1]+1 == pair[0]:
            ...
            preds = self.last_layer(self.second_last_layer(feat_combines))
            scores = preds[:, :, 0]
            gold_label = torch.tensor([cur_comb_label.index(1)])

            cur_loss = self.loss_func(scores, gold_label)
            mention_pair_loss_list[comb_idx] = cur_loss

        mention_pair_loss_list.requires_grad = True
        mention_pair_loss = torch.mean(mention_pair_loss_list, dim=0)
        # print(loss_list)
        token_class_feat = self.token_type_classifier(mention_feat).squeeze()
        token_label = token_label.squeeze()
        token_type_loss = self.loss_func(token_class_feat, token_label)
