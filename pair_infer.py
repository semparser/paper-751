...
def joint_infer(logs, pairs, concepts, instances):
    ...
    # file_path = '/Users/yintonghuo/PycharmProjects/DocLog/coref/output.csv'

    # df_file = pd.read_csv(file_path)
    # info = load_info(df_file)
    ...
    # df_file.loc[:, 'real_pair'] = pd.Series(real_pair)
    # df_file.loc[:, 'left_concept'] = pd.Series(left_concept)
    # df_file.loc[:, 'left_instance'] = pd.Series(left_instance)
    # df_file.loc[:, 'conceptualized'] = pd.Series(conceptualized)
    # df_file.loc[:, 'params'] = pd.Series(params)
